...
const getVendorAnalytics = async (req, res) => {
    try {
        ...
        if (!vendorProfile) {
            return res.status(403).json({ message: "Vendor profile not found. You are not authorized to view analytics." });
        }
        ...
        if (analyticsResult.length > 0) {
            ...
            report = {
                ...
                averageRating: result.averageRating ? result.averageRating.toFixed(2) : 0,
                averagePrepTime: result.averagePrepTime ? result.averagePrepTime.toFixed(2) : 0
            };
        }
