...
const getPayoutHistory = async (req, res) => {
  try {
    ...
    const history = await Payout.find({ vendor: vendorProfile._id }).sort({
      createdAt: -1,
    });
...
const calculateCurrentPayout = async (req, res) => {
  try {
    ...
    const startDate = lastPayout
      ? lastPayout.periodEndDate
      : vendorProfile.createdAt; // Start from last payout or vendor creation date
