...
export async function getAnalytics(startDate, endDate) {
  ...
  const res=await apiFetch(`${BASE_URL}/api/v1/vendors/analytics?${params.toString()}`);
  return res;
}
...
export async function exportAnalytics(startDate, endDate) {
  ...
  if (startDate) params.set('startDate', startDate);
  if (endDate) params.set('endDate', endDate);
  return apiFetch(`${BASE_URL}/api/v1/vendors/analytics/export?${params.toString()}`, { responseType: 'blob' });
}

/** ------------------- Orders ------------------- **/

export async function getVendorOrders({ status, search, sortKey, sortDir } = {}) {
  const params = new URLSearchParams();
...
export async function createMenuItem(payload) {
  ...
  return apiFetch(`${BASE_URL}/api/v1/vendors/menu`, { method: 'POST', body: form });
}
...
export async function deleteMenuItem(itemId) {
  return apiFetch(`${BASE_URL}/api/v1/vendors/menu/${itemId}`, { method: 'DELETE' });
}
...
export async function uploadDocument(documentType, file) {
  ...
  return apiFetch(`${BASE_URL}/api/v1/vendors/documents`, { method: 'POST', body: form });
}

export async function replaceDocument(documentId, file) {
  const form = new FormData();
  form.append('document', file);
  return apiFetch(`${BASE_URL}/api/v1/vendors/documents/${documentId}`, { method: 'PUT', body: form });
}
